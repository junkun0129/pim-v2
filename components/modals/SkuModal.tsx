...
export default function SkuModal({ isOpen, onClose, onSave, dataMap, sku }: SkuModalProps) {
    const [name, setName] = useState(sku?.name || '');
    const [skuId, setSkuId] = useState(sku?.skuId || '');
    const [barcode, setBarcode] = useState(sku?.barcode || '');
    const [price, setPrice] = useState<string>(sku?.price ? String(sku.price) : '');
    const [seriesId, setSeriesId] = useState(sku?.seriesId || '');
    const [categoryIds, setCategoryIds] = useState<string[]>(sku?.categoryIds || []);
    const [attributeSetIds, setAttributeSetIds] = useState<string[]>(sku?.attributeSetIds || []);
    const [attributeValues, setAttributeValues] = useState<Record<string, string>>(sku?.attributeValues || {});
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={sku ? 'SKUを編集' : '新規SKUを追加'}>
            <div className="space-y-4">
                <Input label="SKU名" value={name} onChange={(e) => setName(e.target.value)} placeholder="例: iPhone 14 Pro" required />
                <div className="grid grid-cols-2 gap-4">
                    <Input label="SKU ID" value={skuId} onChange={(e) => setSkuId(e.target.value)} placeholder="例: IP14P-128-BLK" required />
                    <Input label="バーコード (JAN/EAN)" value={barcode} onChange={(e) => setBarcode(e.target.value)} placeholder="例: 4549995359787" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input label="販売価格 (¥)" value={price} onChange={(e) => setPrice(e.target.value)} type="number" placeholder="0" />
                </div>
