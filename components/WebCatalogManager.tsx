...
export default function WebCatalogManager({ catalogs, skus, categories, series, onSaveCatalog, onDeleteCatalog }: WebCatalogManagerProps) {
    ...
    const [editCorner, setEditCorner] = useState<'SHARP' | 'ROUNDED' | 'PILL'>('ROUNDED');
    ...
    const handleAddSection = (type: CatalogSection['type']) => {
        const newSection: CatalogSection = {
            id: `sec-${Date.now()}`,
            type,
            title: type === 'HERO' ? 'Hero Title' : type === 'GRID_CATEGORY' ? 'Category Name' : type === 'RICH_TEXT' ? '見出しを入力' : type === 'VIDEO' ? 'Featured Video' : 'Product Spotlight',
            ...
            content: type === 'RICH_TEXT' ? 'ここにテキストを入力してください。ブランドのストーリーや商品の詳細など、自由な文章で魅力を伝えましょう。' : undefined,
    ...
    const renderPreviewContent = () => {
        // Theme Styles
        const fontClass = editFont === 'SERIF' ? 'font-serif' : editFont === 'MONO' ? 'font-mono' : 'font-sans';
        const radiusClass = editCorner === 'SHARP' ? 'rounded-none' : editCorner === 'PILL' ? 'rounded-2xl' : 'rounded-lg';
        const btnRadiusClass = editCorner === 'SHARP' ? 'rounded-none' : editCorner === 'PILL' ? 'rounded-full' : 'rounded-lg';
            ...
            <div className={`bg-white min-h-screen ${fontClass}`}>
                 ...
                 <div className="text-white py-4 px-6 shadow-md" style={{ backgroundColor: editColor }}>
                     <div className="w-full max-w-7xl mx-auto flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            {editLogo && <img src={editLogo} alt="Logo" className="h-8 w-8 object-contain bg-white rounded p-0.5" />}
                 ...
                 <div className="w-full max-w-7xl mx-auto pb-20">
                     {editSections.map(section => {
                         ...
                         if (section.type === 'HERO') {
                             return (
                                 <div key={section.id} className={`relative w-full h-80 md:h-[500px] overflow-hidden mb-12 shadow-lg ${editCorner === 'SHARP' ? '' : 'rounded-b-3xl'}`}>
                                     ...
                                     <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent flex flex-col justify-end p-8 md:p-16 text-left">
                                         <h2 className="text-4xl md:text-6xl font-bold text-white mb-4 drop-shadow-md leading-tight">{section.title}</h2>
                                         <p className="text-lg md:text-xl text-slate-100 drop-shadow-sm max-w-2xl">{section.subtitle}</p>
                                         <button 
                         ...
                         if (section.type === 'RICH_TEXT') {
                             return (
                                 <div key={section.id} className="py-16 px-6 max-w-4xl mx-auto text-center">
                                     <h3 className="text-3xl font-bold text-zinc-800 mb-6">{section.title}</h3>
                                     <p className="text-lg text-slate-600 leading-relaxed whitespace-pre-wrap">
                                         {section.content}
                                     </p>
                                     <div className="w-20 h-1 mx-auto mt-8 opacity-50" style={{ backgroundColor: editColor }}></div>
                                 </div>
                             )
                         }
                         ...
                         if (section.type === 'VIDEO') {
                             return (
                                 <div key={section.id} className="py-12 px-6 bg-slate-900 text-white my-12">
                                     <div className="max-w-5xl mx-auto text-center">
                                         <h3 className="text-2xl font-bold mb-8 tracking-widest uppercase">{section.title}</h3>
                                         <div className={`aspect-video w-full bg-black overflow-hidden shadow-2xl border border-zinc-700 ${radiusClass}`}>
                                             {section.videoUrl ? (
                                                 <video src={section.videoUrl} controls className="w-full h-full object-cover" />
                                             ) : (
                                                 <div className="w-full h-full flex items-center justify-center text-zinc-500">Video Placeholder</div>
                                             )}
                                         </div>
                                     </div>
                                 </div>
                             ...
                             return (
                                 <div key={section.id} className="py-12 px-6">
                                     <div className="flex items-center gap-4 mb-8">
                                         <h3 className="text-2xl font-bold text-zinc-800">{section.title}</h3>
                                         <div className="h-px flex-1 bg-slate-200"></div>
                                     </div>
                                     <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                         {catSkus.length > 0 ? catSkus.map(sku => (
                                             <div key={sku.id} className={`group bg-white shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 overflow-hidden ${radiusClass}`}>
                                                 <div className="aspect-[4/5] bg-slate-100 relative overflow-hidden">
                                                     <img src={sku.imageUrl || 'https://placehold.co/300x400'} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={sku.name} />
                                                 ...
                                                 <div className="p-4">
                                                     <h4 className="font-bold text-slate-800 truncate text-sm md:text-base mb-1">{sku.name}</h4>
                                 ...
                                 <div key={section.id} className="py-16 px-6 bg-slate-50 mb-12">
                                     <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
                                         <div className="w-full md:w-1/2">
                                             <div className={`aspect-square overflow-hidden shadow-2xl relative bg-white ${radiusClass}`}>
                                         ...
                                         <div className="w-full md:w-1/2 space-y-8">
                                             <div>
                                                 <p className="text-sm font-bold tracking-widest uppercase mb-2 text-slate-400">Featured Product</p>
                                                 <h3 className="text-3xl md:text-5xl font-bold text-slate-900 leading-tight">{section.title || sku.name}</h3>
                                             </div>
                                             ...
                                             <div className="flex items-center gap-6">
                                                 <p className="text-3xl font-bold" style={{ color: editColor }}>¥{sku.price?.toLocaleString()}</p>
                                                 <button 
                 ...
                 <div className="bg-zinc-900 text-slate-400 py-16 px-6">
                     <div className="w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
                         <div>
                             <h4 className="text-white font-bold mb-4 uppercase tracking-widest text-sm">{editName}</h4>
                             <p className="text-sm leading-loose">We bring quality products to your life.<br/>Experience the difference today.</p>
                         </div>
                         <div>
                             <h4 className="text-white font-bold mb-4 uppercase tracking-widest text-sm">Links</h4>
                        ...
                        <Card key={catalog.id} className="group relative overflow-hidden flex flex-col h-full hover:shadow-xl transition-all duration-300 !p-0 border-0">
                            ...
                            <div className="h-24 w-full relative" style={{ backgroundColor: catalog.themeColor }}>
                                <div className="absolute -bottom-6 left-6 w-12 h-12 rounded-lg bg-white dark:bg-zinc-800 shadow-md flex items-center justify-center border border-slate-100 dark:border-zinc-700 z-10 overflow-hidden">
                                    {catalog.logoUrl ? (
                                        <img src={catalog.logoUrl} className="w-full h-full object-cover" />
                                    ) : (
                            ...
                            <div className="pt-10 px-6 pb-6 flex-1 flex flex-col">
                                ...
                                <div className="flex gap-2 mb-4">
                                    <Badge color="gray" className="text-[10px]">{catalog.fontFamily}</Badge>
                                    <Badge color="gray" className="text-[10px]">{catalog.cornerStyle}</Badge>
                                </div>
                ...
                <div className="w-full md:w-80 bg-white dark:bg-zinc-900 border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden shrink-0">
                    ...
                    <div className="flex border-b border-zinc-100 dark:border-zinc-800">
                        <button 
                            ...
                            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === 'CONTENT' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            構成 (Sections)
                        </button>
                        <button 
                            onClick={() => setActiveTab('DESIGN')}
                            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === 'DESIGN' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            デザイン (Design)
                        </button>
                    </div>
                        ...
                        {activeTab === 'DESIGN' ? (
                            ...
                            <div className="space-y-6">
                                <div className="space-y-4">
                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">基本情報</h3>
                                    <Input label="カタログ名" value={editName} onChange={(e) => setEditName(e.target.value)} />
                                    <Input label="説明 (内部用)" value={editDesc} onChange={(e) => setEditDesc(e.target.value)} />
                                </div>

                                <div className="border-t border-slate-100 dark:border-zinc-800 my-4"></div>

                                <div className="space-y-4">
                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">ブランディング</h3>
                                    <div>
                                    ...
                                    <div>
                                        <Input label="ロゴURL" value={editLogo} onChange={(e) => setEditLogo(e.target.value)} placeholder="https://..." />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">スタイル</h3>
                                    <Select label="フォント" value={editFont} onChange={(e) => setEditFont(e.target.value as any)}>
                                        <option value="SANS">Sans Serif (Modern)</option>
                                        <option value="SERIF">Serif (Elegant)</option>
                                        <option value="MONO">Monospace (Technical)</option>
                                    </Select>
                                    <Select label="角の形状 (Radius)" value={editCorner} onChange={(e) => setEditCorner(e.target.value as any)}>
                            ...
                            // --- CONTENT TAB ---
                            <div>
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">セクション一覧</h3>
                                    ...
                                    {editSections.map((section, index) => (
                                        <div key={section.id} className="bg-slate-50 dark:bg-zinc-800 rounded-lg p-3 border border-slate-200 dark:border-zinc-700 group">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {(section.type === 'GRID_CATEGORY' || section.type === 'SPOTLIGHT_SKU') && (
                                                    <select 
                                ...
                                <div className="space-y-2">
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">セクション追加</p>
                                    <div className="grid grid-cols-2 gap-2">
                                        <Button size="sm" variant="secondary" onClick={() => handleAddSection('HERO')} className="justify-start text-xs">
                                            🖼️ ヒーロー
                                        </Button>
                                        <Button size="sm" variant="secondary" onClick={() => handleAddSection('RICH_TEXT')} className="justify-start text-xs">
                                            📝 テキスト
                                        </Button>
                                        ...
                                        <Button size="sm" variant="secondary" onClick={() => handleAddSection('VIDEO')} className="justify-start text-xs col-span-2">
                                            🎥 動画埋め込み
                                        </Button>
                                    </div>
                                </div>
