...
export default function SkuDetailView({ sku, dataMap, onBack, onEdit }: SkuDetailViewProps) {
            ...
            default: // IMAGE or DESIGN
                return (
                    <div key={asset.id} className="group relative aspect-square bg-slate-100 dark:bg-zinc-800 rounded-md overflow-hidden border border-slate-200 dark:border-zinc-700">
                        ...
                        {asset.type === 'DESIGN' && (
                            <div className="absolute top-2 right-2 bg-purple-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">POP</div>
                        )}
                    </div>
                );
        }
                    ...
                    <Card>
                        ...
                        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                            <div className="border-b border-zinc-100 dark:border-zinc-800 pb-2">
                                <dt className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">名前</dt>
                                <dd className="text-slate-900 dark:text-white font-medium">{sku.name}</dd>
                            </div>
                            
                            <div className="border-b border-zinc-100 dark:border-zinc-800 pb-2">
                                <dt className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">SKU ID</dt>
                                <dd className="text-slate-900 dark:text-white font-mono">{sku.skuId}</dd>
                            </div>

                            <div className="border-b border-zinc-100 dark:border-zinc-800 pb-2">
                                <dt className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">バーコード (JAN/EAN)</dt>
                            ...
                            <div className="border-b border-zinc-100 dark:border-zinc-800 pb-2">
                                <dt className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">シリーズ</dt>
                         ...
                         <div className="space-y-0 divide-y divide-zinc-100 dark:divide-zinc-800 border rounded-lg border-zinc-200 dark:border-zinc-700">
                             {allAttributeIds.length > 0 ? (
                                allAttributeIds.map(attrId => (
                                    <div key={attrId} className="flex justify-between items-center p-3">
                                        <span className="font-medium text-slate-500 text-sm">{getAttributeName(attrId)}</span>
                                        <span className="font-semibold text-slate-800 dark:text-white text-sm">{attributeSource.attributeValues[attrId] || 'N/A'}</span>
                                    </div>
                                ))
                             ) : (
                          ...
                          {series && allAttributeIds.length > 0 && 
                            <div className="pt-2 mt-2 flex justify-end">
                                <Badge color="purple" className="text-xs">一部の値はシリーズから継承されています</Badge>
                            </div>
                          }
                    </Card>
                </div>
