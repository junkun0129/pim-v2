...
export default function CreativeStudio({ skus, branches, onSaveAsset }: CreativeStudioProps) {
    ...
    const addElement = (type: ElementType, config: Partial<DesignElement> = {}) => {
        ...
        <div className="flex flex-1 overflow-hidden min-h-0 relative h-full">
            ...
            >
                <div className="w-72 h-full overflow-y-auto custom-scrollbar p-4 flex flex-col gap-6">
                        ...
                        <div className="flex gap-2">
                            {paperColors.map(c => (
                                <button
                                    ...
                                    className={`w-6 h-6 rounded-full border shadow-sm ${paperColor === c.value ? 'border-zinc-900 ring-2 ring-zinc-200 dark:ring-zinc-700' : 'border-slate-200'}`}
                    ...
                    {/* Manual Tools */}
                    <div>
                        <h3 className="font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-wider mb-3">スマート挿入</h3>
                        <div className="space-y-2">
                            <Button variant="secondary" onClick={addSkuName} disabled={!selectedSku} className="w-full justify-start text-xs !py-2">
                                <span className="mr-2">📝</span> 商品名
                            </Button>
                            <Button variant="secondary" onClick={addSkuPrice} disabled={!selectedSku} className="w-full justify-start text-xs font-bold text-red-600 !py-2">
                                <span className="mr-2">¥</span> 価格
                            </Button>
                            <Button variant="secondary" onClick={addSkuImage} disabled={!selectedSku?.imageUrl} className="w-full justify-start text-xs !py-2">
                                <span className="mr-2">🖼️</span> 商品画像
                            </Button>
                            <Button variant="secondary" onClick={addSkuBarcode} disabled={!selectedSku} className="w-full justify-start text-xs font-mono !py-2">
                                <span className="mr-2">|||</span> バーコード
                            </Button>
                        </div>
                        
                        <h3 className="font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-wider mt-5 mb-3">基本図形</h3>
                        <div className="grid grid-cols-2 gap-2">
                            <Button variant="secondary" onClick={() => addElement('RECT')} className="text-xs">□ 四角</Button>
                            <Button variant="secondary" onClick={() => addElement('CIRCLE')} className="text-xs">○ 円</Button>
                            <Button variant="secondary" onClick={() => addElement('TEXT')} className="text-xs col-span-2">T テキスト</Button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <button
                ...
                className={`absolute top-1/2 -translate-y-1/2 z-20 flex items-center justify-center w-6 h-12 bg-white dark:bg-zinc-800 border-y border-r border-zinc-200 dark:border-zinc-700 rounded-r-lg shadow-md hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-400 hover:text-zinc-600 transition-all duration-300 ${isLeftPanelOpen ? 'left-72' : 'left-0'}`}
                ...
                <div className="relative shadow-2xl bg-white transition-transform duration-200" style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT }}>
                    ...
                    >
                        {elements.map(el => {
                            ...
                            const cursorStyle = isSelected ? (isDragging ? 'grabbing' : 'grab') : 'pointer';
            ...
            <button
                ...
                className={`absolute top-1/2 -translate-y-1/2 z-20 flex items-center justify-center w-6 h-12 bg-white dark:bg-zinc-800 border-y border-l border-zinc-200 dark:border-zinc-700 rounded-l-lg shadow-md hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-400 hover:text-zinc-600 transition-all duration-300 ${isRightPanelOpen ? 'right-72' : 'right-0'}`}
            ...
            <div 
                className={`bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 flex flex-col transition-all duration-300 ease-in-out z-10 ${isRightPanelOpen ? 'w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}
            >
                <div className="w-72 h-full overflow-y-auto custom-scrollbar p-4 flex flex-col gap-4">
                    <h3 className="font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-wider">プロパティ</h3>
                    
                    {selectedElement ? (
                        <div className="space-y-4">
                            <div className="text-[10px] text-slate-400 font-mono mb-2 bg-slate-50 dark:bg-zinc-800 p-1 rounded">ID: {selectedElement.id}</div>
                            
                            {(selectedElement.type === 'RECT' || selectedElement.type === 'CIRCLE' || selectedElement.type === 'TEXT') && (
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 mb-1">塗りつぶし色</label>
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="text-xs font-mono text-slate-600 dark:text-slate-300">{selectedElement.fill}</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {selectedElement.type === 'TEXT' && (
                                <>
                                    ...
                                    />
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">スタイル</label>
                                        <div className="flex gap-2">
                                                <button 
                                                onClick={() => updateSelectedProperty('fontWeight', selectedElement.fontWeight === 'bold' ? 'normal' : 'bold')}
                                                className={`flex-1 py-1.5 text-xs border rounded ${selectedElement.fontWeight === 'bold' ? 'bg-zinc-800 text-white border-zinc-800' : 'bg-white text-zinc-600 border-zinc-300'}`}
                                                >
                                                B Bold
                                                </button>
                                        </div>
                                ...
                                {(selectedElement.type === 'RECT' || selectedElement.type === 'CIRCLE' || selectedElement.type === 'IMAGE') && (
                                <div className="grid grid-cols-2 gap-2">
                                    <Input 
                                        label="幅 (W)" 
                                        type="number" 
    ...
    const renderHistoryView = () => (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-4">
            {skuAssets.length > 0 ? (
                skuAssets.map((asset) => (
                    <Card key={asset.id} className="!p-0 overflow-hidden flex flex-col group relative">
                        ...
                        <div className="p-4 bg-white dark:bg-zinc-900">
                            <h4 className="font-bold text-sm text-slate-800 dark:text-white truncate mb-1" title={asset.name}>
                                {asset.name}
                            </h4>
                ...
                <div className="col-span-full flex flex-col items-center justify-center py-20 text-slate-400">
                    ...
                    <p className="text-sm mt-2">デザインタブから新しいPOPを作成してください。</p>
                </div>
            )}
        </div>
    );
             ...
             <div className="bg-white dark:bg-zinc-900 px-4 py-2 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shrink-0 gap-4 overflow-x-auto">
                ...
                <div className="flex items-center gap-2 shrink-0">
                     ...
                     <div className="flex items-center gap-2 bg-slate-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1.5 h-9 relative group">
                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">店舗</span>
                        <select
                        ...
                        >
                         ...
                         <svg className="h-3 w-3 text-zinc-400 absolute right-2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                    </div>

                    {/* SKU Selector */}
                    <div className="flex items-center gap-2 bg-slate-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1.5 h-9 relative group">
                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">SKU</span>
                        <select 
                            value={selectedSkuId} 
                    ...
                    {activeTab === 'DESIGN' && (
                        <>
                            ...
                            <div className="flex items-center gap-2 bg-slate-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2 py-1.5 h-9 w-32">
                                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">価格</span>
                            ...
                            <Button onClick={handleSave} disabled={!selectedSkuId} size="sm" className="h-9 whitespace-nowrap shadow-none">
                                保存
                            </Button>
                        </>
                    )}
