    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black font-sans p-4 relative overflow-hidden group">
      ...
      <div className="w-full max-w-md bg-white/70 dark:bg-zinc-900/60 backdrop-blur-xl rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] border border-white/50 dark:border-white/10 p-8 md:p-10 relative z-10 animate-fade-in-up transition-all duration-500 hover:shadow-[0_8px_40px_0_rgba(31,38,135,0.1)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button
            type="submit"
            className="w-full py-3.5 text-base font-bold shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all hover:scale-[1.02]"
          >
            {isLoading ? (
        ...
        <div className="mt-10 pt-8 border-t border-zinc-200/50 dark:border-zinc-700/50">
          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest text-center mb-6">
            Quick Access (Dev)
          </p>
          <div className="grid grid-cols-1 gap-3">
            {users.map((user, index) => {
              ...
              if (user.id === "user_full") {
                ...
                } else if (user.roleId === "role_creator") {
                  roleLabel = "Creator";
                  ringColor =
                    "ring-orange-500/20 group-hover:ring-orange-500/40";
                } else {
                  roleLabel = "Staff";
                  ringColor = "ring-zinc-500/20 group-hover:ring-zinc-500/40";
                }
              ...
              return (
                ...
                >
                  <div
                    className={`p-0.5 rounded-full ring-2 ${ringColor} transition-all`}
                  >
                    <img
                      src={user.avatarUrl}
                  ...
                  <div className="ml-4 flex-1 text-left">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        {extraLabel && (
                          <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300">
                            {extraLabel}
                          </span>
                        )}
                        <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-700/50 text-zinc-500">
                          {roleLabel}
                        </span>
                      </div>
                    </div>
                  ...
                  <svg
                    className="w-4 h-4 text-zinc-300 group-hover:text-indigo-500 transition-colors transform group-hover:translate-x-1 duration-300"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
