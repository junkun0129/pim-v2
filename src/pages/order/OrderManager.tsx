...
interface OrderManagerProps {
  ...
  onTransferStock: (
    transfer: Omit<StockTransfer, "id" | "status" | "date">
  ) => void;
...
export default function OrderManager({
  ...
  const branchInventoryDisplay = useMemo(() => {
    return skus.map((sku) => {
      const stockRecord = inventory.find(
        (inv) => inv.skuId === sku.id && inv.branchId === currentBranchId
      );
  ...
  const branchTransfers = useMemo(() => {
    return transfers.filter(
      (t) =>
        t.fromBranchId === currentBranchId || t.toBranchId === currentBranchId
    );
  ...
  const getDriverName = (id?: string) =>
    drivers.find((d) => d.id === id)?.name || "未割り当て";
