...
export default function SkuPage({
  ...
  const filteredSkus = useMemo(() => {
    return skus.filter((sku) => {
      // 1. Basic Filters
      const nameMatch = sku.name
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const skuIdMatch = sku.skuId
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const categoryMatch = categoryFilter
        ? sku.categoryIds.includes(categoryFilter)
        : true;
      ...
      if (attributeFilters.length > 0) {
        const matchesAllAttributes = attributeFilters.every((filter) => {
          ...
          const parentSeries = dataMap.series.find(
            (s) => s.id === sku.seriesId
          );
          const inheritedValue =
            parentSeries?.attributeValues[filter.attributeId];
  ...
  const isAllPageSelected =
    ...
    paginatedSkus.every((s) => selectedSkuIds.has(s.id));
  ...
  const handleGenerateTemplate = () => {
    ...
    targetSkus.forEach((sku) => {
      // Direct attributes
      Object.keys(sku.attributeValues).forEach((k) =>
        allUsedAttributeIds.add(k)
      );
      ...
      if (sku.seriesId) {
        ...
        if (ser) {
          Object.keys(ser.attributeValues).forEach((k) =>
            allUsedAttributeIds.add(k)
          );
        }
      }
    ...
    const rows = targetSkus.map((sku) => {
      ...
      const dynamicData = Array.from(allUsedAttributeIds).map((attrId) => {
        ...
        const val =
          sku.attributeValues[attrId] || ser?.attributeValues[attrId] || "";
    ...
    link.setAttribute(
      "download",
      `import_template_${new Date().toISOString().split("T")[0]}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const getAttributeName = (id: string) =>
    dataMap.attributes.find((a) => a.id === id)?.name || id;
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {seriesFilter && (
              <Badge color="purple" className="flex items-center gap-1 pr-1">
                シリーズ:{" "}
                {dataMap.series.find((s) => s.id === seriesFilter)?.name}
                <button
      ...
      {filteredSkus.length > 0 && (
        <div className="sticky bottom-0 z-10 bg-zinc-50/95 dark:bg-black/95 backdrop-blur-md -mx-4 md:-mx-8 px-4 md:px-8 py-4 border-t border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-4 text-sm text-zinc-500">
            <span>
              全 {filteredSkus.length} 件中{" "}
              {(currentPage - 1) * itemsPerPage + 1} -{" "}
              {Math.min(currentPage * itemsPerPage, filteredSkus.length)}{" "}
              件を表示
            </span>
            <select
              value={itemsPerPage}
