...
export default function SeriesDetailView({
  ...
  const getAttributeUnit = (id: string) =>
    dataMap.attributes.find((a) => a.id === id)?.unit || "";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {onEdit && <Button onClick={() => onEdit(series)}>編集</Button>}
      </div>
          ...
          <Card>
            {series.imageUrl ? (
              <img
                src={series.imageUrl}
                alt={series.name}
                className="w-full h-auto object-cover rounded-lg aspect-video mb-4"
              />
            ) : (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(series.attributeValues).map(
                    ([attrId, val]) => {
                      ...
                      return (
                        <div
                          key={attrId}
                          className="flex justify-between items-center border-b border-zinc-100 dark:border-zinc-800 pb-1 last:border-0"
                        >
                  ...
                  {Object.keys(series.attributeValues).length === 0 && (
                    <span className="text-sm text-slate-400">設定なし</span>
                  )}
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-700">
                  {childSkus.length === 0 ? (
                    ...
                    childSkus.map((sku) => (
                      <tr
                        key={sku.id}
                        className="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors cursor-pointer"
                        onClick={() => onViewSku(sku.id)}
                      >
                        <td className="px-4 py-3">
                          <img
                            src={sku.imageUrl || series.imageUrl || ""}
                            alt=""
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            {Object.entries(sku.attributeValues).map(
                              ([attrId, val]) => {
