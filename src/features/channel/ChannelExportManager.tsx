...
export default function ChannelExportManager({
  skus,
  series,
  categories,
  attributes,
  ...
  onUpdateChannel,
  onDeleteChannel,
}: ChannelExportManagerProps) {
  const [viewMode, setViewMode] = useState<"LIST" | "EDIT">("LIST");
  const [editingChannel, setEditingChannel] = useState<ExportChannel | null>(
    null
  );
  ...
  const [targetChannel, setTargetChannel] = useState<ExportChannel | null>(
    null
  );
  ...
  const filteredSkus = useMemo(() => {
    return skus.filter((sku) => {
      ...
      if (attributeSetFilter) {
        ...
        const seriesHasSet =
          parentSeries?.attributeSetIds.includes(attributeSetFilter);
  ...
  const toggleAllFiltered = () => {
    ...
    const allSelected =
      filteredSkus.length > 0 && filteredSkus.every((s) => newSet.has(s.id));
    ...
    if (targetAttrId && targetAttrValue) {
      if (
        !attributeFilters.some(
          (f) => f.attributeId === targetAttrId && f.value === targetAttrValue
        )
      ) {
        setAttributeFilters((prev) => [
          ...prev,
          { attributeId: targetAttrId, value: targetAttrValue },
        ]);
      }
      setTargetAttrValue("");
      setTargetAttrId("");
  ...
  const handleExecuteExport = () => {
    ...
    // Header
    const headerRow = channel.columns
      .map((c) => `"${c.headerName}"`)
      .join(delimiter);
        ...
        .map((col) => {
          ...
          const extractValue = (fieldPath: string) => {
            if (fieldPath.startsWith("attribute.")) {
              ...
              return (
                sku.attributeValues[attrId] ||
                parentSeries?.attributeValues[attrId] ||
                ""
              );
            } else {
              // @ts-ignore
    ...
    const blob = new Blob([bom, content], {
      type: `text/${
        channel.fileFormat === "CSV" ? "csv" : "tab-separated-values"
      };charset=utf-8;`,
    });
    ...
    link.setAttribute(
      "download",
      `${channel.name}_export_${
        new Date().toISOString().split("T")[0]
      }.${channel.fileFormat.toLowerCase()}`
    );
        ...
        {channels.map((channel) => (
          ...
          >
            ...
            <div className="text-sm text-slate-500 mb-6 flex-1 space-y-2">
              <p>
                マッピングカラム数:{" "}
                <span className="font-bold">{channel.columns.length}</span>
              </p>
            ...
            <div className="flex gap-2">
              <Button
                onClick={() => openSelectionModal(channel)}
                className="flex-1"
              >
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-700 pb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleSaveChannel} disabled={!editName}>
            保存
          </Button>
        </div>
      </div>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              異なる属性セットを持つSKUを1つのシートにまとめる場合、予備データソース(Fallback)を設定すると便利です。
            </p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-100 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-medium text-slate-500 w-1/4">
                  内部データソース(優先)
                </th>
                <th className="px-3 py-2 text-left font-medium text-slate-500 w-1/4">
                  予備データソース (Fallback)
                ...
                <tr key={col.id}>
                  ...
                  <td className="px-3 py-2">
                    <input
                      ...
                      onChange={(e) =>
                        updateColumn(col.id, { headerName: e.target.value })
                      }
                  ...
                  <td className="px-3 py-2">
                    <select
                      ...
                      onChange={(e) =>
                        updateColumn(col.id, { sourceField: e.target.value })
                      }
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-600 rounded px-2 py-1"
                    >
                  ...
                  <td className="px-3 py-2">
                    <select
                      ...
                      onChange={(e) =>
                        updateColumn(col.id, {
                          fallbackSourceField: e.target.value,
                        })
                      }
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={`出力対象SKU選択: ${targetChannel?.name}`}
        maxWidth="max-w-5xl"
      >
            ...
            <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 dark:text-slate-300">
              <input
                ...
                checked={
                  ...
                  filteredSkus.every((s) => selectedSkuIds.has(s.id))
                }
              />
                ...
                {paginatedSkus.map((sku) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-xs text-slate-500 truncate max-w-xs">
                      {sku.categoryIds
                        .map((cId) =>
                          getCategoryPath(cId, categories).split(">").pop()
                        )
                        .join(", ")}
                    </td>
                  </tr>
          ...
          <div className="pt-3 border-t dark:border-zinc-700 shrink-0 mt-auto flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            {filteredSkus.length > 0 && (
              <div className="flex items-center gap-2 text-xs text-zinc-500">
                ...
                <span className="ml-1">(全{filteredSkus.length}件)</span>
              </div>
            )}
