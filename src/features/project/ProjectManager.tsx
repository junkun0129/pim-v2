...
import { MOCK_CHAT_MESSAGES, MOCK_IDEAS } from "../src/mockData";
...
interface ProjectManagerProps {
  ...
  drafts: SkuDraft[];
  onCreateDraft: (
    draft: Omit<SkuDraft, "id" | "createdAt" | "authorId">
  ) => void;
...
export default function ProjectManager({
  ...
  const [activeTab, setActiveTab] = useState<"CHAT" | "BRAINSTORM" | "DRAFTS">(
    "CHAT"
  );
  ...
  const getUser = (id: string) =>
    users.find((u) => u.id === id) || { name: "Unknown", avatarUrl: "" };
  ...
  const handleCreateDraft = () => {
    if (selectedProjectId && draftName) {
      ...
      if (linkedIdeaId) {
        setIdeas((prev) =>
          prev.map((i) =>
            i.id === linkedIdeaId ? { ...i, linkedDraftId: "pending" } : i
          )
        );
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <Card
            key={project.id}
            className="group hover:shadow-xl transition-all duration-300 border-l-4 border-l-blue-500 flex flex-col h-full"
          >
  ...
  const renderChat = () => (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-black/20"
      >
                  ...
                  <div
                    className={`px-4 py-2 rounded-2xl text-sm ${
                      isMe
                        ...
                        : "bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-slate-800 dark:text-slate-200 rounded-tl-none shadow-sm"
                    }`}
                  >
        ...
        {isMentionModalOpen && (
          <>
            <div
              ...
              onClick={() => setIsMentionModalOpen(false)}
            ></div>
            <div className="absolute bottom-full left-4 mb-2 w-72 bg-white dark:bg-zinc-800 rounded-lg shadow-xl border border-zinc-200 dark:border-zinc-700 z-40 overflow-hidden flex flex-col max-h-64">
              ...
              <div className="overflow-y-auto p-1">
                ...
                {mentionType === "ALL" && (
                  <>
                    <div className="text-[10px] text-zinc-400 px-2 py-1 mt-1 border-t border-zinc-100 dark:border-zinc-700">
                      アイデア
                    </div>
      ...
      <div className="flex flex-col gap-2 bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 shrink-0">
        <div className="flex gap-2">
          ...
          <div className="flex gap-1">
            <button
              onClick={() => handleAddIdea("yellow")}
              className="w-8 h-10 bg-yellow-200 hover:bg-yellow-300 rounded border border-yellow-300 transition-transform hover:-translate-y-1"
              title="Add Yellow Note"
            ></button>
            <button
              onClick={() => handleAddIdea("blue")}
              className="w-8 h-10 bg-blue-200 hover:bg-blue-300 rounded border border-blue-300 transition-transform hover:-translate-y-1"
              title="Add Blue Note"
            ></button>
            <button
              onClick={() => handleAddIdea("pink")}
              className="w-8 h-10 bg-pink-200 hover:bg-pink-300 rounded border border-pink-300 transition-transform hover:-translate-y-1"
              title="Add Pink Note"
            ></button>
            <button
              onClick={() => handleAddIdea("green")}
              className="w-8 h-10 bg-green-200 hover:bg-green-300 rounded border border-green-300 transition-transform hover:-translate-y-1"
              title="Add Green Note"
            ></button>
          </div>
        </div>
          ...
          {pendingAttachments.length > 0 && (
            <div className="flex gap-2 overflow-x-auto">
              {pendingAttachments.map((f, i) => (
                <span
                  key={i}
                  className="text-xs bg-slate-100 px-2 py-0.5 rounded border border-slate-200 text-slate-600 flex items-center gap-1"
                >
                  {f.name}{" "}
                  <button
                    onClick={() =>
          ...
          {projectIdeas.map((idea) => {
            ...
            return (
              <div
                ...
                className={`${
                  colors[idea.color]
                } p-4 rounded-lg shadow-sm border transform hover:-translate-y-1 transition-transform relative group flex flex-col`}
              >
                ...
                <p className="font-medium text-sm min-h-[40px] whitespace-pre-wrap">
                  {idea.content}
                </p>
                ...
                {idea.attachments && idea.attachments.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {idea.attachments.map((att) => (
                      <div
                        key={att.id}
                        className="rounded overflow-hidden border border-black/10 bg-white/50"
                      >
                        {att.type === "IMAGE" ? (
                          <img
                            src={att.url}
        ...
        {projectDrafts.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700">
            <p className="text-slate-400">
              このプロジェクトに関連付けられたSKUドラフトはまだありません。
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {projectDrafts.map((draft) => {
              ...
              return (
                <div
                  key={draft.id}
                  className="bg-white dark:bg-zinc-900 p-6 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row gap-6"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <Badge color={statusColors[draft.status] as any}>
                        {draft.status}
                      </Badge>
                    ...
                    <div className="flex flex-col gap-2">
                      {draft.status === "PROPOSAL" && (
                        <Button
                          size="sm"
                          onClick={() =>
                            onUpdateDraftStatus(draft.id, "APPROVED")
                          }
                      ...
                      {draft.status === "PROPOSAL" && (
                        <Button
                          ...
                          variant="danger"
                          onClick={() =>
                            onUpdateDraftStatus(draft.id, "REJECTED")
                          }
                          className="w-full"
                        >
                          却下
                      ...
                      {draft.status === "REJECTED" && (
                        <Button
                          ...
                          variant="secondary"
                          onClick={() =>
                            onUpdateDraftStatus(draft.id, "PROPOSAL")
                          }
                          className="w-full"
                        >
                          再申請
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            label="SKU ID (予定)"
            value={draftSkuId}
            onChange={(e) => setDraftSkuId(e.target.value)}
          />
          <Input
            label="販売予定価格"
            type="number"
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            ...
            {users.filter((u) => !selectedProject.memberIds.includes(u.id))
              .length === 0 && (
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button onClick={handleAddMember} disabled={!selectedNewMember}>
              追加
            </Button>
          </div>
        </div>
